...
function createInitialsFromName(name) {
  ...
  let lengthOfName=partsOfName.length;
  if(partsOfName.length===1)
      printInitials+=partsOfName[0].slice(0,2);
  else if(partsOfName.length===2)
      printInitials+=partsOfName[0][0]+partsOfName[1][0];
  else if(partsOfName.length>2)
      printInitials+=partsOfName[0][0]+partsOfName[partsOfName.length-1][0];
  return printInitials.toUpperCase();
}
